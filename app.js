...
window.addEventListener('load', async () => {
    ...
    // 1. Load the updated JSON database (with common names!)
    try {
        const response = await fetch('common_name_plant_database.json');
        plantDatabase = await response.json();
    } catch (error) {
        console.error("Failed to load JSON database:", error);
    }

    // 2. Load the Uncompressed ONNX Model with Mobile Seatbelts
    try {
        ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web/dist/';
        
        // --- MOBILE STABILITY HACKS ---
        ort.env.wasm.numThreads = 1; // Forces the heavy model to run safely on 1 core
        ort.env.wasm.simd = true;    // Unlocks mobile-specific math optimizations
        // ------------------------------
